...
Paths=[Data[i][-2] for i in range(Data.shape[0])]
...
def loadModel(Path):
    ...
    loaded_model.load_weights(Path+"/model_weights.hdf5")
...
@app.route("/")
def template_test():
    return render_template('template.html', label='Please Upload the image first', imagesource='../image/default.jpeg')

@app.route("/map", methods = ['GET', 'POST'])
def map1():
    path1 = request.form['Path']
    Label1 = request.form['Label']
    Coord1 = request.form['Coordinates']
    print(path1)
    print(Label1)
    print(Coord1)
    lattitude=float(Coord1.split(",")[1])
    longitude=float(Coord1.split(",")[0])
    margin=0.01
    print("Map working")
    try:
    ...
    source='image/det_7D6875sample2.jpeg'
    Label='Pigeon'
    return render_template('map.html',address=address,source=path1,Label=Label1,Lat=str(lattitude),Long=str(longitude),Lat1=str(lattitude-margin),Long1=str(longitude-margin),Lat2=str(lattitude+margin),Long2=str(longitude+margin))
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            cv2.imwrite("image/det_"+filename,image)
            os.rename(file_path, os.path.join(app.config['UPLOAD_FOLDER'], filename))
            print("--- %s seconds ---" % str (time.time() - start_time))
            return render_template('template.html', label=label, imagesource='../image/det_' + filename)
...
def predict1(path):
    path1 = "image/"+path
    det=detect1(path1)
    s1 = "  {} birds found  ".format(det)
    s3 = "  {} birds found  Sorry".format(det)
    if det:
	    im=cv2.imread(path1)
	    im1=cv2.resize(im,(224,224))
	    im1=im1.reshape((1,224,224,3))
	    ...
	    with graph.as_default():
    		pred=model.predict(im1)
	    s2 =  Classes[np.argmax(pred,axis=1)][0]
	    return(s1+s2)
    return(s3)
...
app.add_url_rule('/image/<filename>', 'uploaded_file',
                 build_only=True)
app.wsgi_app = SharedDataMiddleware(app.wsgi_app, {
    '/image':  app.config['UPLOAD_FOLDER']
})
